...
function fxfy(data_all)
    ...
    dy = 1.0 # sampling rate

    # Frequency axes
    fx = fftshift(fftfreq(Nx, 1 / Fs))  # frequency in Hz
...
function pattern_metrics(F, data; quantiles = false)
    if quantiles == true
        ...
        # Compute FFT and crop it to Q2–Q3 band
        xmask = (fx .>= q1) .& (fx .<= q3)
        F = F[xmask, :]
    end
...
plot_fft_entropy(data_all, evt = evts_all.continuous)
plot_fft_entropy(data_all, evt = evts_all.condition)
plot_fft_entropy(data_all, evt = evts_all.duration_linear)
